...
class Env:
    ...
    def update_robot_location(self, robot_location):
        robot_location = np.array(robot_location, dtype=float).reshape(2)
        robot_location = np.round(robot_location / NODE_RESOLUTION) * NODE_RESOLUTION
        robot_location = np.round(robot_location, 1)
    ...
    def plot_env(self, step):
        ...
        plt.tight_layout()
        # plt.show()
        plt.savefig('{}/{}_{}_samples.png'.format(gif_path, self.episode_index, step), dpi=150)
        frame = '{}/{}_{}_samples.png'.format(gif_path, self.episode_index, step)
        plt.close()
