...
class Agent:
    ...
    def update_location(self, location):
        # snap to node grid to be consistent with graph keys
        loc = np.array(location, dtype=float).reshape(2)
        loc = np.round(loc / NODE_RESOLUTION) * NODE_RESOLUTION
        loc = np.round(loc, 1)
        self.location = loc
        ...
        # IMPORTANT: use tuple key (same as insert)
        key = (float(loc[0]), float(loc[1]))
        q = self.node_manager.nodes_dict.find(key)

        # node may not exist in this step -> best-effort
        if q is None or getattr(q, "data", None) is None:
            return
    ...
    def update_predict_map(self):
        ...
        # === HARD CONDITION: 预测必须和已观测一致 ===
        belief_map = self.map_info.map
        known = (belief_map != UNKNOWN)  # 已观测（FREE 或 OCCUPIED）
        ...
        for i in range(self.predictor.nsample):
            ...
            x_inpaint_processed = np.where(x_inpaint_processed > 0, FREE, OCCUPIED).astype(np.uint8)

            # 把已观测区域强行写回（关键！）
            x_inpaint_processed[known] = belief_map[known].astype(np.uint8)

            predictions.append(x_inpaint_processed)

        pred_mean = np.mean(predictions, axis=0).astype(np.float32)
        pred_max = np.max(predictions, axis=0).astype(np.uint8)

        # 再保险一次（防止均值/最大化引入偏差）
        pred_mean[known] = belief_map[known].astype(np.float32)
        pred_max[known] = belief_map[known].astype(np.uint8)

        self.pred_mean_map_info = MapInfo(pred_mean,
                                        self.map_info.map_origin_x, self.map_info.map_origin_y, self.cell_size)
        self.pred_max_map_info = MapInfo(pred_max,
                                        self.map_info.map_origin_x, self.map_info.map_origin_y, self.cell_size)
