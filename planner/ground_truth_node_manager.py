...
class GroundTruthNodeManager:
    ...
    def update_graph(self):
        for node in self.node_manager.nodes_dict.__iter__():
            coords = np.around(np.array(node.data.coords, dtype=float), 1)
            key = (float(coords[0]), float(coords[1]))

            gt_q = self.nodes_dict.find(key)
            if gt_q is None:
                # 兜底：node_manager 里出现的点，说明 belief 已经观测为 FREE
                # 那 ground_truth_map_info 也必须允许它存在（至少别崩）
                cell = get_cell_position_from_coords(coords, self.ground_truth_map_info)
                self.ground_truth_map_info.map[cell[1], cell[0]] = FREE

                gt_node = self.add_node_to_dict(coords)
                # 补邻接（否则 Dijkstra/neighbor_set 会出问题）
                gt_node.get_neighbor_nodes(self.ground_truth_map_info, self.nodes_dict)
            else:
                gt_node = gt_q.data

            gt_node.utility = node.data.utility
            gt_node.explored = 1
            gt_node.visited = node.data.visited
