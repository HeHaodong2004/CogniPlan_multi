...
class Node:
    ...
    def update_neighbor_nodes(self, updating_map_info, nodes_dict):
        """
        Robust neighbor update:
        - Always write a decision into neighbor_matrix for each offset:
                1 = free edge (neighbor exists AND collision-free)
                0 = blocked/unknown/missing (neighbor not in dict OR collision)
        - Add symmetric edges when free
        - Stop updating only when the matrix has no -1 left (fully decided)

        This avoids a common failure mode:
        utility drops to 0 early -> need_update_neighbor becomes False -> graph remains edge-sparse ->
        Dijkstra fails -> robot freezes when going to RV.
        """
        center_index = self.neighbor_matrix.shape[0] // 2
        ...
        for i in range(self.neighbor_matrix.shape[0]):
            for j in range(self.neighbor_matrix.shape[1]):
                ...
                # locate neighbor node (must exist in dict to connect)
                neighbor_q = nodes_dict.find((neighbor_coords[0], neighbor_coords[1]))
                if neighbor_q is None:
                    # mark as blocked/unknown for now (important: fill the matrix!)
                    self.neighbor_matrix[i, j] = 0
                    continue

                # collision check (UNKNOWN treated as collision in your check_collision)
                collision = check_collision(self.coords, neighbor_coords, updating_map_info)
                if collision:
                    self.neighbor_matrix[i, j] = 0
                    continue
                ...
                # symmetric update to neighbor
                neighbor_node = neighbor_q.data
                neighbor_matrix_x = center_index + (center_index - i)
                neighbor_matrix_y = center_index + (center_index - j)
        ...
        # stop updating only when all entries are decided
        if np.all(self.neighbor_matrix != -1):
            self.need_update_neighbor = False
